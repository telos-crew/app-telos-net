...
export const GET_ADMIN_STATUS = (state) => {
  const { config: { current_election_id } } = state
  const { status: currentElectionStatus } = state.elections.find(e => e.id === current_election_id)
